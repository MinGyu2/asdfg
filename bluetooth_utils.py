...
def start_le_advertising(sock, min_interval=1000, max_interval=1000,
    ...
    #--start
    dd = (0x04,0x08,0x72,0x48,0x00)
    dd_len = len(dd)
    cmd_pkt = struct.pack("<B%dB" % dd_len, dd_len, *dd)
    #bluez.hci_send_cmd(sock, OGF_LE_CTL, 0x0009, cmd_pkt)

    #cmd_pkt = struct.pack("<B", 0x00)
    #bluez.hci_send_cmd(sock, OGF_LE_CTL, OCF_LE_SET_ADVERTISE_ENABLE, cmd_pkt)

    #cmd_pkt = struct.pack("<B",0x1)
    #bluez.hci_send_cmd(sock, OGF_LE_CTL, 0x000A, cmd_pkt)
    print("ss")
    #--end
def change_advert(sock,data):
    data_length = len(data)
    if data_length > 31:
        raise ValueError("data is too long (%d but max is 31 bytes)",
                         data_length)
